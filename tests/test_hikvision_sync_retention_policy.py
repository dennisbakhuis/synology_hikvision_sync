...
class TestHikvisionSyncRetentionPolicy:
    ...
    def test_retention_disabled(self):
        ...
        sync = HikvisionSync(cameras=[("/test/src", "/test/dst", "test_cam")], retention_days=0)
        
        with patch.object(sync, 'log') as mock_log:
            result = sync.apply_retention_policy()
            
            assert result == 0
            mock_log.assert_called_once_with("Retention policy disabled (retention_days <= 0)")
    
    def test_retention_no_cameras_exist(self):
    ...
    def test_retention_no_old_files(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(sync, 'log') as mock_log:
                with patch.object(sync, 'cameras', [(None, temp_dir, "cam")]):
                    ...
                    assert mock_log.call_count == 2
                    assert "no files needed deletion" in mock_log.call_args_list[-1][0][0]
    
    def test_retention_deletes_old_files(self):
        """Test retention policy deleting old files"""
        sync = HikvisionSync(cameras=[("/test/src", "/test/dst", "test_cam")], retention_days=30)
    ...
    def test_retention_mixed_file_ages(self):
    ...
    def test_retention_file_deletion_error(self):
    ...
    def test_retention_ignores_non_files(self):
...
class TestRetentionPolicyExceptionHandling:
    ...
    def test_retention_camera_processing_exception(self):
        ...
        with patch.object(sync, 'log') as mock_log:
            ...
            with patch('sync_hikvision_cameras.Path') as mock_path:
                ...
                mock_instance.__truediv__ = Mock(side_effect=Exception("Processing error"))
                ...
                assert result == 0
                log_calls = [call[0][0] for call in mock_log.call_args_list]
                assert any("Error processing retention for camera test_cam: Processing error" in call for call in log_calls)