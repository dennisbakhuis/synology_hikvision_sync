...
class TestHikvisionSyncRun:
    ...
    def test_run_lock_acquisition_failure(self):
        ...
        with patch.object(sync, 'acquire_lock', return_value=False):
            with patch.object(sync, 'log') as mock_log:
                ...
                assert result == 1
                mock_log.assert_called_with("Another instance is already running, exiting")
    ...
    def test_run_successful_execution(self):
        ...
        with patch.object(sync, 'acquire_lock', return_value=True):
            with patch.object(sync, 'create_directories'):
                with patch.object(sync, 'process_camera', return_value=mock_camera_stats['test_cam']):
    ...
    def test_run_keyboard_interrupt(self):
        ...
        with patch.object(sync, 'acquire_lock', return_value=True):
            with patch.object(sync, 'create_directories', side_effect=KeyboardInterrupt()):
                with patch.object(sync, 'release_lock'):
                    with patch.object(sync, 'log') as mock_log:
                        result = sync.run()
    ...
    def test_run_unexpected_exception(self):
        ...
        with patch.object(sync, 'acquire_lock', return_value=True):
            with patch.object(sync, 'create_directories', side_effect=Exception("Test error")):
                with patch.object(sync, 'release_lock'):
                    with patch.object(sync, 'log') as mock_log:
                        result = sync.run()
    ...
    def test_run_always_releases_lock(self):
