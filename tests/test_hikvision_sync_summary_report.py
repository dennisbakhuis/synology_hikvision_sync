...
class TestHikvisionSyncSummaryReport:
    ...
    def test_generate_summary_report_single_camera(self) -> None:
        ...
        with patch.object(sync, "log") as mock_log:
            ...
            assert any(
                "test_cam: Videos 5/10, Images 4/8" in call for call in log_calls
            )
            assert any(
                "Total: 9 new, 5 existing, 18 segments" in call for call in log_calls
            )
            assert any("Efficiency: 27.8% skipped" in call for call in log_calls)

    def test_generate_summary_report_multiple_cameras(self) -> None:
        ...
        with patch.object(sync, "log") as mock_log:
            ...
            assert any(
                "Total: 11 new, 5 existing, 20 segments" in call for call in log_calls
            )
            assert any("Retention: 5 old files deleted" in call for call in log_calls)

    def test_generate_summary_report_no_segments(self) -> None:
        ...
        with patch.object(sync, "log") as mock_log:
            ...
            assert any(
                "Total: 0 new, 0 existing, 0 segments" in call for call in log_calls
            )
