...
class HikvisionScheduler:
    ...
    def run_scheduled(self) -> int:
        ...
        try:
            ...
            while self.running:
                log_message(f"Waiting {self.interval_minutes} minutes until next sync...")
...
class HikvisionSync:
    def __init__(
        self, 
        cameras: list[tuple[Path | str, Path | str, str]] | None = None, 
        lock_file: str | None = None, 
        cache_directory: str | None = None, 
        retention_days: int | None = None
    ) -> None:
        if cameras is None:
            raise ValueError("No cameras provided. Use discover_cameras() to find cameras or provide explicit camera configuration.")
    ...
    def apply_retention_policy(self) -> int:
        ...
        self.log(f"Applying retention policy: deleting files older than {self.retention_days} days")
                    ...
                    for file_path in subdirectory_path.iterdir():
                        ...
                        try:
                            ...
                            if file_modification_time < cutoff_timestamp:
                                ...
                                self.log(f"Deleted old file: {file_path} ({file_size} bytes) [{camera_tag}]")
        ...
        if total_deleted > 0:
            ...
            self.log(f"Retention cleanup completed: {total_deleted} files deleted, {size_megabytes:.1f} MB freed")
        else:
            self.log("Retention cleanup completed: no files needed deletion")
            
        return total_deleted
            
    def process_camera(self, source_path: Path | str, destination_path: Path | str, camera_tag: str) -> dict[str, dict[str, int]]:
        ...
        """
        self.log(f"Processing camera '{camera_tag}': {source_path} -> {destination_path}")
        
        if not Path(source_path).exists():
            self.log(f"Warning: Source directory does not exist: {source_path}")
            return {'videos': {'total': 0, 'existing': 0, 'new': 0, 'failed': 0}, 
                    'images': {'total': 0, 'existing': 0, 'new': 0, 'failed': 0}}
            
        try:
            video_statistics = self._process_media(source_path, destination_path, camera_tag, 'video')
            image_statistics = self._process_media(source_path, destination_path, camera_tag, 'image')
            
            self.log(f"Completed processing camera {camera_tag}: {video_statistics['new']} videos, {image_statistics['new']} images")
    ...
    def _process_media(self, source_path: Path | str, destination_path: Path | str, camera_tag: str, media_type: str) -> dict[str, int]:
        ...
        try:
            ...
            media_directory = Path(destination_path) / f"{media_type}s" if media_type == 'image' else Path(destination_path) / media_type
            media_directory.mkdir(parents=True, exist_ok=True)
            
            existing_files_set = {file.name for file in media_directory.iterdir() if file.is_file()} if media_directory.exists() else set()
            ...
            for segment_number, segment_data in enumerate(segments_list):
                try:
                    ...
                    timestamp_formatted = segment_start_time.strftime('%Y-%m-%d_%H-%M-%S')
                    output_filename = f"{timestamp_formatted}-{camera_tag}.{file_extension}"
                    ...
                    if media_type == 'video':
                        extraction_result = hikvision_instance.extractSegmentMP4(segment_number, cachePath=self.cache_directory, filename=destination_file_path)
                    else:
                        extraction_result = hikvision_instance.extractSegmentJPG(segment_number, cachePath=self.cache_directory, filename=destination_file_path)
                    
                    if extraction_result and os.path.exists(destination_file_path) and os.path.getsize(destination_file_path) > 0:
                        self.log(f"Extracted {media_type}: {destination_file_path} [{camera_tag}]")
                        new_files_count += 1
            ...
            if existing_files_count > 0:
                self.log(f"Skipped {existing_files_count} existing {media_type} files for camera {camera_tag}")
            
            return {'total': len(segments_list), 'existing': existing_files_count, 'new': new_files_count, 'failed': failed_files_count}
            
        except Exception as e:
            self.log(f"Error processing {media_type}s for camera {camera_tag}: {e}")
            return {'total': 0, 'existing': 0, 'new': 0, 'failed': 0}
    
    def _generate_summary_report(self, all_camera_statistics: dict[str, dict[str, dict[str, int]]], deleted_files_count: int) -> None:
        ...
        summary_totals = {'videos': {'total': 0, 'existing': 0, 'new': 0, 'failed': 0}, 
                         'images': {'total': 0, 'existing': 0, 'new': 0, 'failed': 0}}
        
        for camera_tag, camera_statistics in all_camera_statistics.items():
            self.log(f"{camera_tag}: Videos {camera_statistics['videos']['new']}/{camera_statistics['videos']['total']}, Images {camera_statistics['images']['new']}/{camera_statistics['images']['total']}")
            for media_type in ['videos', 'images']:
                for statistic_key in summary_totals[media_type]:
                    summary_totals[media_type][statistic_key] += camera_statistics[media_type][statistic_key]
        
        total_segments_processed = summary_totals['videos']['total'] + summary_totals['images']['total']
        total_new_files = summary_totals['videos']['new'] + summary_totals['images']['new']
        total_existing_files = summary_totals['videos']['existing'] + summary_totals['images']['existing']
        
        self.log(f"Total: {total_new_files} new, {total_existing_files} existing, {total_segments_processed} segments")
        if deleted_files_count: 
            self.log(f"Retention: {deleted_files_count} old files deleted")
        if total_segments_processed: 
            self.log(f"Efficiency: {(total_existing_files/total_segments_processed)*100:.1f}% skipped")
        self.log("=" * 50)
    ...
    def run(self) -> int:
        ...
        try:
            ...
            for source_path, destination_path, camera_tag in self.cameras:
                all_camera_statistics[camera_tag] = self.process_camera(source_path, destination_path, camera_tag)
...
def create_argument_parser() -> argparse.ArgumentParser:  # pragma: no cover
    ...
    parser.add_argument(
        '--sync-interval', '-s',
        type=int,
        default=int(os.getenv('SYNC_INTERVAL_MINUTES', str(DEFAULT_SYNC_INTERVAL_MINUTES))),
...
if __name__ == '__main__':  # pragma: no cover
    ...
    if args.sync_interval < 1 or args.sync_interval > 1440:
        parser.error(f"sync-interval must be between 1 and 1440 minutes, got: {args.sync_interval}")
    ...
    if Path(args.input_dir).exists():
        ...
        print(f"Input directory {args.input_dir} not found and no cameras could be discovered")
        print("Please ensure the input directory exists or provide explicit camera configuration")
        sys.exit(1)
